...
export class MockRootApiService
{
    ...
    socketService() {
        return app.serviceRegistry.resolveService<MockWebSocketService>({kind: 'socket'});
    }

    ruleService() {
        return app.serviceRegistry.resolveService<MockRuleService>({kind: 'rule'});
    }

    markerService() {
        return app.serviceRegistry.resolveService<MockMarkerService>({kind: 'marker'});
    }
    ...
    miscService() {
        return app.serviceRegistry.resolveService<MockMiscService>({ kind: 'misc' })
    }
}
